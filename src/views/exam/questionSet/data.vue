<template>
  <div :class="{ hasTagsView: needTagsView }" class="main-content-body">
    <transition :enter-active-class="proxy?.animate.searchAnimate.enter"
                :leave-active-class="proxy?.animate.searchAnimate.leave">
      <div v-show="showSearch" ref="searchContainer" class="search">
        <el-form ref="queryFormRef" :inline="true" :model="queryParams">
          <el-form-item label="作者" prop="author">
            <el-input v-model="queryParams.author" clearable placeholder="请输入作者" style="width: 240px"
          ...
          <el-form-item label="难易程度" prop="difficulty">
            <el-select v-model="queryParams.difficultyName" clearable placeholder="难易程度" style="width: 240px"
                       @change="handleQuery">
    ...
    <el-card class="card-content" shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['exam:question:add']" icon="Plus" plain type="primary" @click="handleAdd">新增
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['exam:question:edit']" :disabled="single" icon="Edit" plain type="success"
                       @click="handleUpdate()">批量修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['exam:question:remove']" :disabled="multiple" icon="Delete" plain type="danger"
                       @click="handleDelete()">删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['exam:question:export']" icon="Download" plain type="warning" @click="handleExport">
              导出
            </el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList" @resetQuery="resetQuery"
                         :columns="columns"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="questionList" :height="tableHeight"
                @selection-change="handleSelectionChange">
        <el-table-column align="center" fixed type="selection" width="55" />
        <el-table-column v-if="columns[0].visible" align="center" label="题目ID" prop="questionId" width="200" />
        <el-table-column v-if="columns[1].visible" align="center" label="题库ID" prop="questionSetId" width="200" />
        <el-table-column v-if="columns[2].visible" align="center" label="题库名称" prop="questionSet.questionSetName"
                         width="200" />
        <el-table-column v-if="columns[3].visible" align="center" label="题型" prop="questionType" width="100">
          <template #default="scope">
            <dict-tag :options="exam_question_type" :value="scope.row.questionType" />
          </template>
        </el-table-column>
        <el-table-column v-if="columns[4].visible" align="center" label="难易程度" prop="difficultyName" width="100" />
        <el-table-column v-if="columns[5].visible" :show-overflow-tooltip="true" align="center" label="题目标题"
                         prop="content" width="300" />
        <el-table-column v-if="columns[6].visible" :show-overflow-tooltip="true" align="center" label="答案" prop=""
                         width="300">
          <template #default="{ row }">
            <span v-if="row.templateObj">
              {{ getCorrectAnswer(row.templateObj) }}
            </span>
          </template>
        </el-table-column>
        <el-table-column v-if="columns[7].visible" align="center" label="作者" prop="author" width="150" />
        <el-table-column v-if="columns[8].visible" align="center" label="分值" prop="score"
                         width="80" />
        <el-table-column v-if="columns[9].visible" align="center" label="题目标签" prop="tag" width="200">
          <template #default="scope">
            <el-tag v-if="scope.row.tagsNames===null" type="info">无</el-tag>
            <el-tag
              v-for="(tag,index) in scope.row.tagsNames"
              v-else :key="tag"
        ...
        <el-table-column v-if="columns[10].visible" align="center" label="显示顺序" prop="orderNum" width="100" />
        <el-table-column v-if="columns[11].visible" align="center" label="创建时间" prop="createTime" width="200" />
        <el-table-column align="center" class-name="small-padding fixed-width" fixed="right" label="操作" width="130">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['exam:question:edit']" circle icon="Edit" plain type="primary"
                         @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['exam:question:remove']" circle icon="Delete" plain type="danger"
    ...
    <!-- 添加或修改题目对话框 -->
    <el-drawer v-model="dialog.visible" :append-to-body="true" :show-close="false" :with-header="false"
               size="100%" class="full-drawer">
      <el-container class="app-wrapper">
        <el-aside v-if="dialog.title==='添加题目'" class="sidebar-container drawer-left-menu">
            ...
            <el-scrollbar>
              <el-card shadow="hover">
                ...
                >
                      ...
                      >
                        <div v-for="(item,i) in question.templateObj.children"
                          ...
                          <textarea v-if="question.templateObj.type==='Textarea'"
                                    ...
                                    type="textarea" />
                          <span
                            v-if="item.attribute.examCorrectAnswer===item.id && question.templateObj.attribute.examAnswerMode!=='select' && question.templateObj.attribute.examAnswerMode!=='selectCorrect'  && question.templateObj.type!=='Textarea' && question.templateObj.type!=='MultipleBlank' "
                  ...
                  <div v-if="questionFormList.length===0 && viewFlag" class="m-auto" v-html="DragDemonstration">
                  </div>
                </VueDraggable>
...
const columns = ref<FieldOption[]>([
  { key: 0, label: `题目ID`, visible: false, children: [] },
  { key: 1, label: `题库ID`, visible: false, children: [] },
  { key: 2, label: `题库名称`, visible: true, children: [] },
  { key: 3, label: `题型`, visible: true, children: [] },
  { key: 4, label: `难易程度`, visible: true, children: [] },
  { key: 5, label: `题目标题`, visible: true, children: [] },
  { key: 6, label: `答案`, visible: true, children: [] },
  { key: 7, label: "作者", visible: true, children: [] },
  { key: 8, label: `分值`, visible: true, children: [] },
  { key: 9, label: `题目标签`, visible: true, children: [] },
  { key: 10, label: `显示顺序`, visible: false, children: [] },
  { key: 11, label: `创建事件`, visible: true, children: [] }
]);
...
const defaultQuestionSet = ref<string | number>();
/** 查询题目详细 */
const getQuestions = async () => {
  const { data } = await getQuestionSet(props.questionSetId);
  queryParams.value.questionSetId = data.questionSetId;
  defaultQuestionSet.value = data.questionSetId;
  await getList();
};
...
const resetQuery = () => {
  ...
  queryParams.value.difficultyName = undefined;
...
const handleUpdate = async (row?: QuestionVO) => {
  ...
  const res = await getQuestionList(_questionIds);
  Object.assign(questionFormList.value, res.data);
  questionFormList.value.forEach((item: any) => {
    /*将题目ID进行替换*/
    item["templateObj"]["id"] = item.questionId;
    /*添加填空题title*/
    if (item["templateObj"]["type"] === "MultipleBlank" || item["templateObj"]["type"] === "Textarea") {
      let i = 1;
      item["templateObj"]["children"].forEach((x: any) => {
        x["title"] = "填空" + i;
        i++;
      });
    }
...
const handleDelete = async (row?: QuestionVO) => {
  ...
  await delQuestion(_questionIds);
...
const calculateTableHeight = () => {
  ...
  tableHeight.value = screenHeight.value - 50 - tagsViewHeight.value - 26 - searchHeight.value - 185;
};
watch(showSearch, () => {
  calculateTableHeight();
});

// 使用 watch 来观察 questionSetId 的变化
watch(() => props.questionSetId, (newVal, oldVal) => {

<template>
  <div :class="{ hasTagsView: needTagsView }" class="main-content">
      ...
      <el-col :lg="20" :xs="24">
        ...
        <el-card class="card-content" shadow="hover">
          <template #header>
            <el-row :gutter="10">
              ...
              <right-toolbar v-model:showSearch="showSearch" :columns="columns" :search="true"
                             @queryTable="getList" @resetQuery="resetQuery"></right-toolbar>
            </el-row>
          </template>
          ...
          <el-table v-loading="loading" :data="userList" :height="tableHeight"
            ...
            <el-table-column v-if="columns[0].visible" key="userId" align="center" fixed label="用户编号" prop="userId"
                             width="150"/>
            <el-table-column v-if="columns[1].visible" key="userName" :show-overflow-tooltip="true" align="center" fixed
                             label="用户名称"
                             prop="userName"
                             width="150"/>
            ...
            <el-table-column v-if="columns[7].visible" key="roleIds" align="center" label="角色" prop="roleIds"
                             width="200">
              <template #default="scope">
                <dict-tag :options="sys_user_role_ids" :value="scope.row.roleIds"/>
              </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column v-if="columns[5].visible" key="status" align="center" label="状态" width="100">
              <template #default="scope">
            ...
            <el-table-column v-if="columns[6].visible" align="center" label="创建时间" prop="createTime" width="200">
              <template #default="scope">
            ...
            <el-table-column align="center" class-name="small-padding fixed-width" fixed="right" label="操作"
              ...
              <template #default="scope">
                <el-tooltip v-if="scope.row.userId !== 1" content="修改" placement="top">
                  <el-button v-hasPermi="['system:user:edit']" circle icon="Edit" plain type="primary"
                             @click="handleUpdate(scope.row)"></el-button>
                </el-tooltip>
                <el-tooltip v-if="scope.row.userId !== 1" content="删除" placement="top">
                  <el-button v-hasPermi="['system:user:remove']" circle icon="Delete" plain type="danger"
                             @click="handleDelete(scope.row)"></el-button>
                </el-tooltip>

                <el-tooltip v-if="scope.row.userId !== 1" content="重置密码" placement="top">
                  <el-button v-hasPermi="['system:user:resetPwd']" circle icon="Key" plain type="info"
                             @click="handleResetPwd(scope.row)"></el-button>
                </el-tooltip>

                <el-tooltip v-if="scope.row.userId !== 1" content="分配角色" placement="top">
                  <el-button v-hasPermi="['system:user:edit']" circle icon="Check" plain type="warning"
    ...
    <!-- 添加或修改用户配置对话框 -->
    <el-drawer ref="formDialogRef" v-model="dialog.visible" :title="dialog.title" append-to-body size="30%"
               @close="closeDialog">
...
const {
  ...
  sys_user_sex,
  sys_user_role_ids
} = toRefs<any>(proxy?.useDict('sys_normal_disable', 'sys_user_sex', 'sys_user_role_ids'));
...
const columns = ref<FieldOption[]>([
  ...
  {key: 7, label: '角色', visible: true, children: []}
])

    ...
    <el-row :gutter="20">
      ...
      <el-col :lg="5" :xs="24">
        <el-card class="card-content" shadow="hover">
          <el-input v-model="menuName" clearable placeholder="请输入菜单名称" prefix-icon="Search"/>
            ...
            >
              <template #default="scope">
                <div class="custom-node">
                  <svg-icon :icon-class="scope.data.icon"/>
...
import {addMenu, delMenu, getMenu, listChildMenu, listMenu, menuTreeSelect, updateMenu} from '@/api/system/menu';
...
const menuTreeOptions = ref<MenuVO[]>([]);
...
const getList = async () => {
  loading.value = true
  const res = await listChildMenu(queryParams.value);
...
watchEffect(
  () => {
    menuTreeRef.value?.filter(menuName.value);
  },
  {
